...
#:set TYPES_NAMES = list(zip(TYPES, NAMES))
    ...
    integer,parameter :: wp = real64  !! local copy of `finterp_rk` with a shorter name
    ...
    type,extends(linear_interp_class),public :: linear_interp_1d
        ...
        procedure,public :: evaluate   => interp_1d_real
        procedure,public :: evaluate_derivative => interp_1d_derivative
    ...
    interface dintrv
        module procedure :: dintrv_real, dintrv_dual
    end interface
...
!  1D linear interpolation routine.
    ...
    class(linear_interp_1d),intent(inout) :: me
    ${TYPE1}$,intent(in)                   :: x
    ${TYPE1}$,intent(out)                  :: f     !! Interpolated \( f(x) \)
