...
contains
  ...
  subroutine compute_gibbs_energy(temperature, nz, nsp, gibbs_energy)
    ...
    use photochem_vars, only: int_nz ! work arrays
  ...
  subroutine compute_photolysis_rates(nz, nsp, kj, nw, xs_x_qy, densities, wavl, flux, &
    ...
    real(real_kind), intent(in) :: usol(nz,nsp)
  ...
  subroutine right_hand_side(nsp, nq, usol)
    ! bring into scope things like T, density, etc.
    
    ! basically if it DOES NOT depend on usol. What is in the atmosphere.
    ! then it can be computed before the right hand side
  end subroutine
  ...
  subroutine photochem_equilibrium
    ...
    call stuff_before_integration(T,D) !etc.
